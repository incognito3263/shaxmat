...
const THEMES: Record<string, { light: string; dark: string }> = {
  ...
  classic: { light: '#ebecd0', dark: '#739552' }, // Chess.com Green
  wood: { light: '#f0d9b5', dark: '#b58863' }, // Chess.com Wood/Brown
  forest: { light: '#eeeed2', dark: '#769656' }, 
  ocean: { light: '#dee3e6', dark: '#8ca2ad' }, // Icy Sea
  midnight: { light: '#c3c6be', dark: '#727fa2' }, // Slate
}
...
function Square({ square, light, isSelected, isLegal, isCapture, isKingInCheck, isLastMove, piece, theme, col, row, showCol, showRow }: any) {
  ...
  return (
    <div className={`relative aspect-square flex items-center justify-center cursor-pointer ${isKingInCheck ? 'check-pulse' : ''}`} style={{ backgroundColor: bgColor }} onClick={() => !isSpectator && useGameStore.getState().selectSquare(square)}>
      ...
      {isSelected && <div className="absolute inset-0 pointer-events-none" style={{ backgroundColor: 'var(--selected-gold)' }} />}
      
      {/* Legal Move Indicators (Chess.com style dots/rings) */}
      {isLegal && !piece && <div className="w-[30%] h-[30%] rounded-full pointer-events-none z-10" style={{ backgroundColor: 'var(--legal-dot)' }} />}

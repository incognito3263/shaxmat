      ...
      <div className="flex-1 overflow-y-auto space-y-2.5 pr-1 custom-scrollbar" style={{ maxHeight: '350px' }}>
        {pendingRequests?.map((req) => (
          <div key={req.id} className="p-2 rounded-lg bg-[var(--accent-cyan)]/5 border border-[var(--accent-cyan)]/10 flex items-center justify-between">
            ...
            <div className="flex gap-1.5"><button onClick={() => respondToFriendRequest(req.id, true)} className="p-1 rounded-md bg-[var(--accent-cyan)] text-black"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><polyline points="20 6 9 17 4 12"></polyline></svg></button></div>
          </div>
        ))}
        {friends.map((f) => (
...
function ModeSelection() {
  ...
  return (
    <div className="flex-1 w-full max-w-[1200px] mx-auto p-6 md:p-10 lg:p-12">
      <div className="flex justify-between items-center mb-12 border-b border-[var(--border)] pb-8"><div className="flex items-center gap-4"><Logo size="md" /><h1 className="text-xl font-bold uppercase tracking-widest text-[var(--accent-white)]">Arena</h1></div><div className="flex gap-4 items-center"><LanguageSwitcher /><ThemeSwitcher /></div></div>
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
        <div className="lg:col-span-5 space-y-8">
          <div className="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-8 flex flex-col items-center text-center shadow-sm relative overflow-hidden"><div className="absolute top-0 right-0 p-4"><button onClick={() => setIsEditModalOpen(true)} className="text-[10px] uppercase font-bold text-[var(--text-muted)] hover:text-[var(--accent-cyan)] transition-colors tracking-widest">Settings</button></div><Avatar src={user?.avatar || '👤'} size="xl" /><h2 className="text-2xl font-bold mt-4 mb-1 text-[var(--accent-white)] tracking-tight">{user?.username}</h2><div className="text-[10px] text-[var(--accent-cyan)] font-bold uppercase tracking-[0.3em] mb-6 px-3 py-1 bg-[var(--accent-cyan)]/5 rounded-full border border-[var(--accent-cyan)]/10">{user ? getUserTitle(user.wins, t) : ''}</div><div className="grid grid-cols-2 gap-4 w-full mb-8"><div className="bg-[var(--bg)] p-4 rounded-lg border border-[var(--border)]"><div className="text-[9px] uppercase tracking-widest text-[var(--text-muted)] mb-1">Win Rate</div><div className="text-xl font-bold text-green-500">{winRate}%</div></div><div className="bg-[var(--bg)] p-4 rounded-lg border border-[var(--border)]"><div className="text-[9px] uppercase tracking-widest text-[var(--text-muted)] mb-1">Games</div><div className="text-xl font-bold">{totalGames}</div></div></div><div className="w-full flex items-center justify-center gap-3 bg-[var(--bg)] py-3 px-4 rounded-lg border border-[var(--border)] group cursor-pointer hover:border-[var(--accent-cyan)]/30 transition-all" onClick={() => { navigator.clipboard.writeText(user?.public_id || ''); setNotification({ text: t.idCopied, type: 'success' }); }}><span className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest">ID:</span><span className="text-base font-mono font-bold text-[var(--accent-cyan)] tracking-widest">{user?.public_id}</span><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="opacity-30 group-hover:opacity-100 transition-opacity"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg></div><button onClick={logout} className="mt-6 text-[10px] font-bold text-[var(--text-muted)] hover:text-[var(--check-red)] transition-colors uppercase tracking-[0.2em]">{t.logout}</button></div>
          <div className="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-8 shadow-sm"><div className="mb-8"><div className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest mb-4">⏱️ {t.timeControl}</div><div className="grid grid-cols-4 gap-2">{[{l:'1m',s:60,i:0},{l:'1+1',s:60,i:1},{l:'3m',s:180,i:0},{l:'3+2',s:180,i:2},{l:'5m',s:300,i:0},{l:'10m',s:600,i:0},{l:'30m',s:1800,i:0}].map(tc => ( <button key={tc.l} onClick={() => { setTimeLimit(tc.s); setTimeIncrement(tc.i); }} className={`py-2 rounded-md text-[10px] font-bold transition-all border ${timeLimit === tc.s && timeIncrement === tc.i ? 'bg-[var(--accent-cyan)] text-black border-[var(--accent-cyan)]' : 'bg-[var(--bg)] text-[var(--text-muted)] border-[var(--border)] hover:border-[var(--text-main)]'}`}>{tc.l.toUpperCase()}</button> ))}</div></div><div className="space-y-4"><div className="bg-[var(--bg)] p-5 rounded-lg border border-[var(--border)] hover:border-[var(--accent-cyan)]/30 transition-all"><div className="flex items-center justify-between mb-4"><span className="text-[11px] font-bold uppercase tracking-widest">🤖 {t.singlePlayer}</span><div className="flex gap-1.5">{['easy', 'normal', 'hard'].map(d => <button key={d} onClick={() => setDifficulty(d)} className={`px-2 py-1 rounded text-[8px] font-bold uppercase tracking-tighter transition-all ${difficulty === d ? 'bg-[var(--accent-cyan)] text-black' : 'text-[var(--text-muted)] hover:text-[var(--text-main)]'}`}>{d}</button>)}</div></div><button onClick={() => createGame('AI', undefined, difficulty, timeLimit, timeIncrement)} className="w-full py-3 bg-[var(--accent-cyan)] text-black font-bold rounded-md uppercase tracking-widest text-[10px] hover:brightness-105 active:scale-[0.98] transition-all">Battle AI</button></div><button onClick={startMatchmaking} className="w-full p-5 bg-[var(--bg)] border border-[var(--border)] rounded-lg hover:border-[var(--accent-cyan)]/30 transition-all flex items-center justify-between group"><span className="text-[11px] font-bold uppercase tracking-widest">🌍 {t.quickPlay}</span><span className="text-[var(--accent-cyan)] group-hover:translate-x-1 transition-transform">→</span></button><div className="bg-[var(--bg)] p-5 rounded-lg border border-[var(--border)]"><div className="text-[11px] font-bold uppercase tracking-widest mb-4 text-left">👤 {t.multiplayer}</div>{!foundUser ? ( <div className="flex gap-2"><input type="text" maxLength={8} value={opponentId} onChange={e => setOpponentId(e.target.value.replace(/\D/g, ''))} placeholder="Opponent ID" className="flex-1 bg-[var(--surface)] border border-[var(--border)] rounded-md px-4 py-2 text-xs text-center font-mono outline-none focus:border-[var(--accent-cyan)]" /><button onClick={async () => { if (opponentId.length === 8) { if (opponentId === user?.public_id) { setNotification({ text: t.selfPlayError, type: 'error' }); return } const target = await searchUser(opponentId); if (target) setFoundUser(target); else setNotification({ text: t.userNotFound, type: 'error' }); } }} className="bg-[var(--accent-cyan)] text-black px-5 py-2 rounded-md font-bold uppercase text-[10px] hover:brightness-105 transition-all">{t.go}</button></div> ) : ( <div className="flex items-center gap-3"><Avatar src={foundUser.avatar} size="sm" /><div className="flex-1 text-left"><div className="text-xs font-bold">{foundUser.username}</div><div className="text-[9px] text-[var(--text-muted)] font-mono">ID: {foundUser.public_id}</div></div><div className="flex gap-1.5"><button onClick={() => sendInvite(foundUser.public_id, timeLimit, timeIncrement)} className="px-3 py-1.5 bg-[var(--accent-cyan)] text-black text-[9px] font-bold uppercase rounded transition-all">{t.inviteFriend}</button><button onClick={() => { setFoundUser(null); setOpponentId(''); }} className="p-1.5 bg-[var(--surface-hover)] text-[var(--text-muted)] rounded">✕</button></div></div> )}</div></div></div>
        </div>
...
function PlayerBadge({ color, isActive }: { color: 'white' | 'black'; isActive: boolean }) {
  ...
  return (
    <div className={`flex items-center gap-3 px-4 py-3 rounded-lg border transition-all ${isActive ? (isWhite ? 'bg-white/5 border-white/20' : 'bg-[var(--accent-cyan)]/5 border-[var(--accent-cyan)]/30') : 'bg-transparent border-transparent'}`}><Avatar src={avatar || (isWhite ? '♔' : '♚')} size="sm" /><div className="flex-1 min-w-0"><div className="text-[11px] font-bold uppercase tracking-widest truncate" style={{ color: isWhite ? '#fff' : 'var(--accent-cyan)' }}>{isWhite ? t.white : t.black}</div><div className="flex flex-wrap gap-0.5 mt-1 opacity-40">{capturedPieces.map((p, i) => <span key={i} className="text-[10px]">{p}</span>)}</div></div><div className={`px-3 py-1 rounded font-mono text-sm font-bold ${isActive ? (timeLeft < 30 ? 'bg-red-500 text-white animate-pulse' : 'bg-[var(--surface-2)] text-[var(--accent-cyan)]') : 'text-[var(--text-muted)] opacity-50'}`}>{formatTime(timeLeft)}</div></div>
  )
}
...
export default function App() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[var(--bg)]">
      ...
      <main className="flex-1 flex flex-col lg:flex-row gap-8 p-8 max-w-[1400px] mx-auto w-full items-start"><aside className="w-full lg:w-64 flex flex-col gap-6 order-2 lg:order-1">{game && <div className="space-y-3"><PlayerBadge color="black" isActive={game.turn === 'black' && !isGameOver} /><PlayerBadge color="white" isActive={game.turn === 'white' && !isGameOver} /></div>}<GameControls /></aside><section className="flex-1 flex items-start justify-center order-1 lg:order-2 p-2 bg-[var(--surface)] border border-[var(--border)] rounded-xl shadow-sm"><Board /></section><aside className="w-full lg:w-72 order-3 flex flex-col gap-6"><MoveHistory />{game?.game_mode === 'Person' && !isSpectator && <Chat />}</aside></main>
